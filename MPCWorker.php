...
class MPCWorker
{
    ...
    protected function __construct(array $params)
    {
        ...
        // if a mode was selected, override everything with its settings
        if ($this->params['mode']) {
            foreach ($this->params['modes'][$this->params['mode']] as $k => $v) {
                $this->params[$k] = $v;
            }
        }
    ...
    public static function runFromCLIArguments($argv)
    {
        ...
        while ($myargs) {
            ...
            case '--mode': case '-o':
                if (!($arg = array_shift($myargs))
                  || !array_key_exists($arg, $params['modes'])
                ) {
                    die("Mode parameter was missing or doesn't exist.\n");
                }
                $params['mode'] = $arg;
                break;
            case '--help': case '-?':
...
";
    ...
    public function latest()
    {
        ...
        foreach ($target as &$t) {
            ...
            // $disp = $x['disp'];
            // $disp = str_replace($this->params['relativeTo'] . '/', '', $x['name']);
            // if (!$this->params['simpleOut']) {
            //     $disp = sprintf('%5d', $i+1) . ". $disp";
            //     if (isset($x['mtime'])) $disp .= ' [' . date('Y-m-d', $x['mtime']) . ']';
            // }
        }
